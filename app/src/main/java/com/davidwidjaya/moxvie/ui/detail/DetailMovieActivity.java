package com.davidwidjaya.moxvie.ui.detail;
...
public class DetailMovieActivity extends AppCompatActivity {
    ...
    ArrayList<SearchResponse> arrSearch;

    int id;
    MovieResponse tempObjMovie;
    TvResponse tempObjTv;
    SearchResponse tempObjSearch;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (i.getExtras() != null) {
            ...
            if (i.hasExtra("arrMovie")) {
                new LoadIsFavoriteMovie().execute(); //cek apa sudah di favorite sebelumnya
                section = "arrMovie";
            ...
            } else if (i.hasExtra("arrSearch")) {
                new LoadIsFavoriteTv().execute();
                section = "arrMovie";

                arrSearch = i.getParcelableArrayListExtra("arrSearch");
                for (int j = 0; j < arrSearch.size(); j++) {
                    if (id == arrSearch.get(j).getId()) {
                        tempObjSearch = arrSearch.get(j);
                    }
                }

                Picasso.get().load(tempObjSearch.getBackdrop()).fit().centerInside().into(activityDetailMovieBinding.ivBackdropActivityDetailMovie);
                Picasso.get().load(tempObjSearch.getImageUrl()).fit().centerInside().into(activityDetailMovieBinding.ivPoster);
                activityDetailMovieBinding.tvActivityDetailMovieName.setText(tempObjSearch.getName());
                activityDetailMovieBinding.tvActivityDetailMoviePopularity.setText(tempObjSearch.getPopularity() + " Views");
                activityDetailMovieBinding.tvActivityDetailMovieRating.setText(tempObjSearch.getRating() + "");
                activityDetailMovieBinding.tvActivityDetailMovieRelease.setText(tempObjSearch.getRelease());
                activityDetailMovieBinding.tvActivityDetailMovieOverview.setText(tempObjSearch.getOverview());

                if (tempObjSearch.getMovieTv().equals("movie")) {
                    section = "arrMovie";
                } else if (tempObjSearch.getMovieTv().equals("tv")) {
                    section = "arrTv";
                }
            }
        }
            ...
            public void onClick(View v) {
                if (tempObjTv != null && section.equals("arrTv")) {
                    ...
                    if (isFavorite) {
                        ...
                        new DeleteTvTask().execute(tempTv);
                    } else {
                        //add
                        new AddTvTask().execute(tempTv);
                    }
                } else if (tempObjMovie != null && section.equals("arrMovie")) {
                    ...
                    if (isFavorite) {
                        //jika sudah di favorit
                        new DeleteMovieTask().execute(tempMovie);
                    } else {
                        //add
                        new AddMovieTask().execute(tempMovie);
                    }
                } else if (tempObjSearch != null) {
                    if (section.equals("arrMovie")) {
                        tempMovie = new Movie(tempObjSearch.getId(), tempObjSearch.getImageUrl(), tempObjSearch.getName(), tempObjSearch.getRating(), tempObjSearch.getBackdrop(), tempObjSearch.getRelease(), tempObjSearch.getOverview(), tempObjSearch.getPopularity());
                        if (isFavorite) {
                            //jika sudah di favorit
                            new DeleteMovieTask().execute(tempMovie);
                        } else {
                    ...
                    } else if (section.equals("arrTv")) {
                        tempTv = new Tv(tempObjSearch.getId(), tempObjSearch.getImageUrl(), tempObjSearch.getName(), tempObjSearch.getRating(), tempObjSearch.getBackdrop(), tempObjSearch.getRelease(), tempObjSearch.getOverview(), tempObjSearch.getPopularity());
                        if (isFavorite) {
                            //jika sudah di favorit
                            new DeleteTvTask().execute(tempTv);
                        } else {
    ...
    private class AddTvTask extends AsyncTask<Tv, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            Toast.makeText(DetailMovieActivity.this, "Success Favorited", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private class DeleteTvTask extends AsyncTask<Tv, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            isFavorite = false;
            setFavoriteState(isFavorite);
            Toast.makeText(DetailMovieActivity.this, "Success Deleted", Toast.LENGTH_SHORT).show();
        }
    }
